...
describe('client api', () => {
  ...
  it('uses the prompt, prime, profile, coach, session, and import endpoints', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(6, '/api/coach', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(7, '/api/paragraph-result', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(8, '/api/sessions', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(9, '/api/vocab/import', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(10, '/api/vocab/capture', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(11, '/api/vocab/save', expect.objectContaining({ method: 'POST' }));
  });
});
