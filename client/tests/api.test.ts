...
describe('client api', () => {
  ...
  it('uses the prompt, prime, profile, coach, session, and import endpoints', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(10, '/api/coach', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(11, '/api/paragraph-result', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(12, '/api/sessions', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(13, '/api/vocab/import', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(14, '/api/vocab/capture', expect.objectContaining({ method: 'POST' }));
    expect(fetchMock).toHaveBeenNthCalledWith(15, '/api/vocab/save', expect.objectContaining({ method: 'POST' }));
  });
});
